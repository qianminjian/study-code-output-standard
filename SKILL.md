...
### Step -1 · 规模检测与模式路由（v2.6 新增）

**目标**：在 Step 0 启动确认前，自动检测目标项目的 Java 文件数，决定用 v3.0 串行还是 v3.5 并行。避免 > 500 文件项目在 v3.0 模式下 agent context 溢出导致 stall。

**检测命令**：
```bash
JAVA_COUNT=$(find ${PROJECT_ROOT} -name "*.java" -not -path "*/test/*" | wc -l | tr -d ' ')
```
...
| Java 文件数 | 推荐模式 | 原因 |
|:----------:|:------:|------|
| ≤ 200 | **v3.0 串行** | 单 agent 轻松覆盖 |
| 201-500 | **v3.0 串行（全量）** | 可行，但要求 Read ≤ 500 行/文件 |
| 501-1000 | **v3.5 并行 + 分层抽样** | 单 agent 超载风险；Read 上限 200 文件/agent |
| > 1000 | **v3.5 并行 + 分层抽样（必用）** | 单 agent 必然 stall；24 模块实测 4/28 agent 在此区间失速 |

**分层抽样协议**（当 Java > 500 时自动启用）：

| Tier | 文件类型 | 采样率 | 说明 |
|------|---------|:---:|------|
| 1 | Controller / Config / Security / Filter | **100%** | API 层必全量 |
| 2 | Service / ServiceImpl | **50%** | 深度分析，结构全量 + 内容抽样 |
| 3 | Mapper / Entity / XML | **30%** | 数据层抽样，结构 grep 全量 |
| 4 | Handler / Utils / VO / DTO | **20%** | 工具层代表性覆盖 |

**分层抽样关键约束**：
- 每个变体 Read 总数 ≤ 200 文件（v3.0 全量）/ ≤ 80 文件（v3.5 per worker）
- **资产 @meta 块必须标注采样率**：`"sample_rate": "Tier: 1=100%, 2=50%, 3=30%, 4=20%"`
- 采样资产与全量资产**必须可区分**（CI 校验时检查 @meta sample_rate 字段）

**给用户的提示**（Step 0 时显示）：
```
检测到目标项目有 {N} 个 Java 文件（{LOC} 行），推荐 v3.5 并行 + 分层抽样模式。
预计产出 {N*0.3} 篇资产（采样率标注在 @meta 中）。
是否继续？ [Y/n]
```
...
### C/D/E. 4 阶段管线 · Worker 认知变体 · Prompt 组装逻辑

**本节已迁移至 `references/parallel-mode.md`**（约 220 行）：

- §C 4 阶段管线（Phase 1/2/3/4 + 编排者自写 01/00）
- §D 4 种 Worker 认知变体（Extract / Analyze / Synthesize / Aggregate + 5 铁律 + 4 扫描流程）
- §E 编排者 Prompt 组装逻辑（5 变量解析 + 变体 → 资产映射表 + Worker Spawn 通用 Prompt 模板 + Sonnet 合并 worker）

读 `references/parallel-mode.md` 的 §C/§D/§E 后继续下方 §F（Phase 间交接机制）。
...
```markdown
## Phase {N} 完成 @ {ISO-8601 timestamp}

### {资产编号} ({资产名})
- {关键实体 1}：{数字 1} 个 / {数字 2} 张
- {关键实体 2}：{数字 1}
- {风险/发现}：{概述}
- Anomaly：{单资产看不全的异常（Phase 4 校验用）}
...
## 跨资产交叉一致性标记（Phase 4 校验用）
- [ ] 02 entity 名 vs 03 controller 引用 → 必须 100% 一致
- [ ] 03 端点 vs 06 公开端点 → 应 ≤ 03 总数
- [ ] 02 推断表 vs 04 Mapper XML namespace → 推断表必须在某 Mapper namespace 出现
- [ ] 06 P0/P1 风险 vs 11 技术债 → 06 列出位置应进入 11 资产

## ⚠️ 跨资产严重度对齐（Phase 1 完成后编排者必做）
**原因**：Phase 1 的 4 个 worker 独立判断严重度，同一问题可能在不同资产中标不同等级
（如 XxlJobInfoMapper.xml:156 的 ${} SQL 注入在 02 标 P2、04 标 P0）。
由此导致 12（修复建议）聚合时严重度混乱。

**协议**：编排者在 Phase 1 完成后，必须：
1. 对照 02/03/04/06 4 个资产中相同位置的 P0/P1 标注
2. 如发现同一 file:line 在不同资产中有不同严重度 → **统一到最高等级**
3. 在下方表格中记录对齐决策

| 问题位置 | 02 资产 | 04 资产 | 06 资产 | **最终** | 理由 |
|---------|:-----:|:-----:|:-----:|:------:|------|
| <文件>:<行号> | P2 | P0 | — | **P0** | `${}` 是 SQL 注入载体，无论类型 |

**违反**：编排者跳过此步骤 → Phase 2-4 启动前检查对齐表是否为空 → 空则**拒绝启动后续 Phase**。
...
**协议（硬性）**：
- **写者**：编排者（仅编排者写，worker 不写）
- **写时机**：每个 Phase 完成后**立即**写，不延后
- **读时机**：后续 Phase 的 worker spawn prompt **必须引用**此文件作为基线
- **格式**：上述 4 段（Phase 完成 / 跨阶段引用 / 累积数字 / 模板字段解释）
- **共享字段**：累积数字一致性段**禁止修改**——单一数据源
- **删除**：assets 全部完成后由编排者清理（不入 git）
- **违反**：worker 不读 .phase-facts.md = 跨资产一致性无保证 = 输出作废
...
> **全文见 `references/constraints.md`**（路径处理 / 方法论定位 / 资产输出位置硬性红线 / CLAUDE.md 双文件策略 / 绝对禁止）。
> orchestrator 违反任何一条 = 本次执行作废。
