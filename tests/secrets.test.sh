...
#   - 排除 .git/、tests/、scripts/、templates/、ai-prompts/
...
#   - references/anti-patterns.md（反例展示）
#   - methodology/07-典型案例与反模式.md（反例展示）
#   - references/asset-types.md（资产类型说明）
...
# 2. 扫 examples/wxcbrc-case.md 中的密钥"huawei"明文（应已脱敏为占位）
echo "[T2] 校验 wxcbrc-case.md 中无 'huawei' 真实密钥"
HUAWEI_HITS=$(grep -rn "huawei" "$REPO_ROOT/examples/wxcbrc-case.md" 2>/dev/null || true)
...
# 3. 校验 P1-04：methodology/07-典型案例与反模式.md 中的密钥占位符（应使用脱敏串，不应保留真实值）
echo "[T3] 校验 07-典型案例与反模式.md 中密钥占位（应使用 <REDACTED> 或脱敏串）"
SECRET_LITERAL=$(grep -nE 'secret\s*=\s*"[a-zA-Z0-9-]{6,}"' "$REPO_ROOT/methodology/07-典型案例与反模式.md" 2>/dev/null || true)
...
FM_SECRETS=$(grep -rnE '"data_source".*password|"data_source".*secret' "$REPO_ROOT/methodology" --include="*.md" 2>/dev/null || true)
