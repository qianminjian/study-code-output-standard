...
bash "$REPO_ROOT/skill/install.sh" --path "$INSTALL_TARGET" </dev/null >/dev/null 2>&1
...
bash "$REPO_ROOT/skill/uninstall.sh" --path "$INSTALL_TARGET" >/dev/null 2>&1
...
set +e
bash "$REPO_ROOT/skill/install.sh" --path </dev/null >/dev/null 2>&1
EXIT_CODE=$?
set -e
...
HELP_OUT="$(bash "$REPO_ROOT/skill/install.sh" --help 2>&1)"
echo "$HELP_OUT" | grep -q "用法" || { echo "FAIL: --help 输出无 '用法'"; exit 1; }
echo "  ✓ 帮助输出正常"

# 6. install --version 应输出版本
echo "[T5] install --version 输出版本"
VERSION_OUT="$(bash "$REPO_ROOT/skill/install.sh" --version 2>&1)"
...
set +e
bash "$REPO_ROOT/skill/uninstall.sh" --path </dev/null >/dev/null 2>&1
EXIT_CODE=$?
set -e
[ "$EXIT_CODE" = "2" ] || { echo "FAIL: 应退出 2，实际 $EXIT_CODE"; exit 1; }
echo "  ✓ 退出码 2"
...
bash "$REPO_ROOT/skill/install.sh" --path "$DRY_TARGET" --dry-run </dev/null >/dev/null 2>&1
