...
LEAKED=$(grep -rn "/Users/minjianq" "$REPO_ROOT" \
  --exclude-dir=".git" --exclude-dir="tests" --exclude-dir="node_modules" --exclude-dir=".v3.5-test" --exclude-dir=".v3.0-test" --exclude-dir="_proc-use" \
...
# 3. 校验 .internal/examples/wxcbrc-case.md 使用 <保密路径> 占位符
echo "[T3] 校验 wxcbrc-case.md 使用 <保密路径> 占位"
EXAMPLE="$REPO_ROOT/.internal/examples/wxcbrc-case.md"
[ -f "$EXAMPLE" ] || { echo "SKIP: wxcbrc-case.md 不在 examples/（已移入 .internal/）"; }
if [ -f "$EXAMPLE" ]; then
