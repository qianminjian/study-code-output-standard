...
Phase 2 ─── 1 worker（05 服务逻辑，可选依赖 03+04 输出）
    ▼
Phase 3 ─── 2 worker 并行（10a+10b，依赖 Phase 1+2 全部）
           10a: 业务流 1-4（数据密集型：Controller→Service→Mapper 单链）
           10b: 业务流 5-N（语义密集型：跨模块协作、外部系统集成）
           **v2.6 新增**：10 拆为 2 并行 → 实测加速 1.5-2x
    ▼
Phase 4 ─── 并行 11+13，然后 12
           编排者收到 10a+10b 后合并为 10-业务流图.md
...
**Phase 3 拆分策略**（v2.6）：
- **10a worker**：负责前 4 条业务流（由编排者根据 Step 1 勘探结果预分配）—— 这些通常是 Controller→Service→Mapper 的直线调用链，数据密集但语义简单
- **10b worker**：负责第 5 条及以后的业务流—— 这些通常是跨模块协作、外部系统集成、复杂状态机，语义密集
- **编排者 merge**：Phase 3 完成后，编排者将 10a+10b 合并为 10-业务流图.md
- **拆分阈值**：当目标模块的 Controller > 50 或端点 > 200 时自动启用（与 §D 单 Worker 上限联动）
- **不拆分场景**：Controller ≤ 50 且端点 ≤ 200 → 10 只有 1 个 worker（减少 merge overhead）
